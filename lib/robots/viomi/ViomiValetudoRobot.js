...
class ViomiValetudoRobot extends MiioValetudoRobot {
    ...
    onCloudConnected() {
        ...
        setTimeout(() => {
            // Trigger robot re-request of map presigned URL
            // This will likely screw up ongoing jobs so we only send it if we're idle
            const stateAttr = this.state.getFirstMatchingAttributeByConstructor(stateAttrs.StatusStateAttribute);
            if (stateAttr === null || stateAttr === undefined || !stateAttr.isActiveState() && stateAttr.value !== stateAttr.StatusStateAttribute.VALUE.PAUSED) {
                this.sendCommand("set_uploadmap", [0], {timeout: 5000}).catch((reason => {
                    Logger.warn("Failed to trigger map upload URL request", reason);
                }));
            }
        }, 5000);
    }
